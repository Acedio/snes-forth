...
\ INCLUDE std.fth into bank 1 so we can use it there.
1 CBANK!
INCLUDE std.fth
0 CBANK!
...
: TEST-STRINGS
  T{ S" HELLO" SWAP C@ -> 5 [CHAR] H }T
;

BANK@ LOWRAM BANK! CREATE TEST-LOWRAM-VAR 1 CELLS ALLOT BANK!
CREATE TEST-VAR 1 CELLS ALLOT

CREATELOWRAM CREATELOWRAM-VAR

: TEST-VARS
  T{ TEST-LOWRAM-VAR 1024 2* 2* 2* < -> TRUE }T
  T{ 21 TEST-LOWRAM-VAR ! TEST-LOWRAM-VAR @ -> 21 }T
  T{ 42 TEST-LOWRAM-VAR ! TEST-LOWRAM-VAR @ -> 42 }T
  T{ CREATELOWRAM-VAR 1024 2* 2* 2* < -> TRUE }T
;
...
1 CBANK!
2121 CONSTANT MY-BANK-CONSTANT
: MY-BANK-COMPILE MY-BANK-CONSTANT ;
: MY-BANK-LOCAL-CALL MY-BANK-COMPILE 101 + ;
: BANK-DATA-BANK BANK@ ;
: BANK-PROGRAM-BANK PHK ;
0 CBANK!

: TEST-BANK-CALL
  T{ BANK1-CALL MY-BANK-COMPILE -> 2121 }T
  T{ BANK1-CALL MY-BANK-LOCAL-CALL -> 2222 }T
  \ 0x7F AND to account for FASTROM.
  T{ BANK1-CALL BANK-DATA-BANK 0x7F AND -> 0 }T
  T{ BANK1-CALL BANK-PROGRAM-BANK 0x7F AND -> 1 }T
;
